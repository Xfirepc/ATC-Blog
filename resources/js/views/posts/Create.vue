    ...
    <div class="max-w-3xl mx-auto">
      <div class="bg-white rounded-lg shadow-lg p-8">
        <div class="text-center mb-8">
          <h2 class="text-3xl font-bold text-gray-900">Crear Nuevo Post</h2>
        </div>
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      try {
        await this.$store.dispatch('posts/createPost', this.form);
        this.$router.push('/posts');
      } catch (error) {
        this.error = error.response?.data?.message || 'Error al crear el post';
      } finally {
        this.loading = false;
      }
    }
